...
export function LocationDetailDialog({ location, onClose }: Props) {
  ...
  const convenienceItems = t('locations.detail.convenienceItems', { returnObjects: true }) as string[];
      ...
      <DialogPrimitive.Portal>
        ...
        >
          <DialogPrimitive.Close
            ...
            aria-label={t('locations.detail.close')}
          >
            <X className="h-4 w-4" />
          </DialogPrimitive.Close>
          ...
          <div className="grid max-h-[calc(100svh-48px)] grid-cols-1 overflow-y-auto md:grid-cols-[0.95fr_1.05fr]">
            <div className="relative min-h-[280px] md:min-h-[560px]">
              ...
              <div className="absolute left-5 top-5 z-10 flex flex-wrap gap-2">
                <Badge className="rounded-full bg-white/90 text-slate-700 backdrop-blur-sm border-0 shadow-sm">
                  ...
                  {t('locations.detail.kmFromAlmaty', { distance: location.distanceFromAlmatyKm })}
                </Badge>
                <Badge className="rounded-full bg-white/90 text-slate-700 backdrop-blur-sm border-0 shadow-sm">
                  {location.region}
                </Badge>
            ...
            <div className="flex flex-col p-6 md:p-8">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-slate-500">
                ...
                <span>{t('locations.detail.kmFromAlmaty', { distance: location.distanceFromAlmatyKm })}</span>
              </div>
              ...
              {features.length > 0 && (
                <div className="mt-6">
                  <h4 className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-slate-400">
                    {t('locations.detail.featuresTitle')}
                  </h4>
              ...
              <div className="mt-6">
                <h4 className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-slate-400">
                  {t('locations.detail.convenienceTitle')}
                </h4>
                <ul className="space-y-2">
                  {Array.isArray(convenienceItems) && convenienceItems.map((item) => (
              ...
              <div className="mt-6 flex flex-col gap-2 sm:flex-row">
                <Link to={ROUTES.CATALOG} className="flex-1" onClick={onClose}>
                  <Button className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-semibold">
                    {t('locations.detail.ctaCatalog')}
                    <ArrowRight className="ml-2 h-4 w-4" />
                  </Button>
                </Link>
                <DialogPrimitive.Close asChild>
                  <Button variant="outline" className="flex-1" onClick={onClose}>
                    {t('locations.detail.close')}
                  </Button>
                </DialogPrimitive.Close>
              </div>
            </div>
