...
export const OrderPage = () => {
  const { t } = useTranslation(['order', 'seo']);
  ...
  return (
    <>
      <Helmet>
        <title>{t('seo:order.title')}</title>
        <meta name="description" content={t('seo:order.description')} />
      </Helmet>
      <OrderLayout currentStep={0}>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-[1fr_380px] md:gap-8">
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-900 mb-3">{t('order:orderPage.accessoriesTitle')}</h3>
              {isLoading ? (
                ...
                <div className="flex flex-col items-center rounded-2xl border border-emerald-900/10 bg-white py-12 text-center">
                  ...
                  <p className="text-sm text-slate-500">{t('order:orderPage.noAccessories')}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 backdrop-blur px-4 py-3">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs text-slate-500">{t('order:orderPage.total')}</p>
